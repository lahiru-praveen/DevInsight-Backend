...
import textwrap
import logging
from utilis.llm_openai.optimizer import CodeReviewByOpenai
from utilis.llm_gemini.optimizer import CodeReviewByGemini
...
@llm_router.post("/get-review-by-openai")
async def get_review(code_context: code_context_data.CodeContextData):  # Receive codeContext from the request body
    try:
        ...
        review,suggestions,refer_link = CodeReviewByOpenai.get_review(input_chunks, code_context.language, code_context.description)
...
@llm_router.post("/get-review-by-gemini")
async def get_review(code_context: code_context_data.CodeContextData):  # Receive codeContext from the request body
    try:
        action_result = await code_db.add_code(code_context)  # Await the asynchronous call
        if not action_result.status:
        ...
        review,suggestions,refer_link = CodeReviewByGemini.get_review(input_chunks, code_context.language, code_context.description)
