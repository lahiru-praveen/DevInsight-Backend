...
#         finally:
#             return action_result
#     async def find_code_by_project_and_file(self, project_name: str, file_name: str) -> dict:
#         try:
#             code_data = await self.__collection.find_one(
#                 {"projectName": project_name, "fileName": file_name},
#                 {"_id": 0, "projectID": 1, "description": 1, "mode": 1}
#             )
#             return code_data
#         except Exception as e:
#             print(f"Error fetching code data: {e}")
#             return None
#
...
class DatabaseConnector:
    ...
    async def add_request(self, entity: dict) -> ActionResult:
        action_result = ActionResult(status=True)
        try:
            get_next_request_id_pipeline = [
                {
            ...
            entity['request_id'] = next_request_id

            result = await self.__collection.insert_one(entity)
            action_result.data = str(result.inserted_id)
            action_result.message = "Insert successful"
        except Exception as e:
            action_result.status = False
